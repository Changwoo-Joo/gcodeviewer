...
def plot_path(coords, is_extrudes, end_index):
    fig = go.Figure()
...
if uploaded_file:
    ...
    if len(coords) < 2:
        ...
        end_index = st.slider("G-code 라인 진행 (%)", min_value=1, max_value=len(coords)-1, value=len(coords)-1)
        fig = plot_path(coords, is_extrudes, end_index)
